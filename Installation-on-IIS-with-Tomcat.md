...
* Configure IIS
  ...
  * Under `Request Filtering` for the parent website hosting the `jakarta` VirtualDirectory. Remove request filters for *.java, *.cs and *.vb. If you do not remove these filters then files viewed in gitbucket with these extensions will give a 404 error. You may wish to remove additional filters too such as .csproj.
